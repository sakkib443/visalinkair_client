...
// ===================================================================
// Shared table for Hajj / Umrah package-booking inquiries.
// Same table + detail modal, differs only by service, title, and icon.
// ===================================================================
...
export default function PackageInquiries({ service, title, prefix, IconComponent }) {
    const inqNo = (id = "") => `${prefix}-${parseInt(id.slice(-6) || "0", 16).toString().padStart(6, "0")}`;
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">
                            Real-time queue of package booking requests.
                        </p>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                ...
                <StatCard label="On page" value={items.length} color="var(--color-brand-blue)" />
                                    ...
                                    return (
                                        <tr key={it._id} className="hover:bg-gray-50/60">
                                            ...
                                            <td className="px-4 py-3 text-sm">
                                                <p className="font-semibold text-brand-dark">{ex.packageName || "—"}</p>
                                                {(ex.duration || ex.pricePerPerson) && (
                                                    <p className="text-[11px] text-gray-500 mt-0.5">
                                                        {ex.duration && <span>{ex.duration}</span>}
                                                        {ex.duration && ex.pricePerPerson && <span className="mx-1">·</span>}
                                                        {ex.pricePerPerson && <span>${ex.pricePerPerson}/pax</span>}
                                                    </p>
                                                )}
                                            </td>
                                            ...
                                            <td className="px-4 py-3">
                                                <div className="flex items-center justify-end gap-1">
                                                    <button onClick={() => setDetail(it)} className="p-2 rounded-lg text-gray-500 hover:bg-blue-50 hover:text-blue-600" title="View">
                                                        <FiEye size={15} />
                                                    </button>
                                                    <button
            ...
            {detail && <DetailModal it={detail} onClose={() => setDetail(null)} inqNo={inqNo} IconComponent={IconComponent} />}
...
function DetailModal({ it, onClose, inqNo, IconComponent }) {
    const ex = it.extra || {};
    const waHref = it.phone
        ? `https://wa.me/${it.phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hi ${it.name}, regarding your package inquiry ${inqNo(it._id)}...`)}`
        : null;
            ...
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                ...
                <div className="p-5 space-y-5">
                    ...
                    <Section title="Package">
                        <Row label="Type">{ex.packageType === "hajj" ? "Hajj" : "Umrah"}</Row>
                        <Row label="Package">{ex.packageName || "—"}</Row>
                        {ex.duration && <Row label="Duration">{ex.duration}</Row>}
                        {ex.hotel && <Row label="Hotel">{ex.hotel}</Row>}
                        {ex.pricePerPerson && <Row label="Price/person">${ex.pricePerPerson}</Row>}
                        <Row label="Persons">{ex.persons || 1}</Row>
                        {ex.persons && ex.pricePerPerson && (
                            <Row label="Est. total"><span className="font-bold text-emerald-700">${(Number(ex.persons) * Number(ex.pricePerPerson)).toLocaleString()}</span></Row>
                        )}
                    </Section>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                        ...
                        {it.email && (
                            <a href={`mailto:${it.email}?subject=Re: Package Inquiry ${inqNo(it._id)}`} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-brand-dark hover:bg-brand-dark-hover text-white text-sm font-bold">
...
function Row({ label, children }) {
    return (
        <div className="flex items-center gap-3 px-3 py-2 text-sm">
            <span className="text-[11px] font-semibold text-gray-500 w-28 flex-shrink-0">{label}</span>
