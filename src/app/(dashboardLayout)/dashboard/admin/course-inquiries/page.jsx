...
// ===================================================================
// Admin — Course Inquiries queue
//
// পাবলিক সাইটের "Get Admission" মোডাল আর কোর্স ডিটেইলস পেজের ইনকয়েরি
// ফর্ম — দুটোই service="course" দিয়ে জমা হয়, সবই এখানে আসে।
// WhatsApp দিয়ে পাঠানো ইনকয়েরিও এখানেই থাকে, শুধু source আলাদা।
// ===================================================================
...
// অন্য ইনকয়েরি পেজগুলোর মতোই — Mongo _id এর শেষ ৬ hex থেকে পড়ার মতো নম্বর
const inqNo = (id = "") => "CRS-" + parseInt(id.slice(-6) || "0", 16).toString().padStart(6, "0");
...
export default function CourseInquiriesPage() {
    ...
    const params = useMemo(() => {
        const p = { page, limit: 20, service: SERVICE };
    ...
    // `load` প্রতি রেন্ডারে নতুন ফাংশন, ডিপ-অ্যারেতে দিলে অসীম লুপ হতো —
    // তাই ইচ্ছে করেই শুধু page/status দেখা হচ্ছে (search আলাদা বাটনে চলে)।
    ...
    const remove = async (id) => {
        if (!confirm("Delete this course inquiry?")) return;
        setBusyId(id);
        try {
    ...
    return (
        <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">
                            Admission requests submitted from the public course pages.
                        </p>
                    </div>
                </div>
                <button
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                ...
                <StatCard label="On this page" value={items.length} color="var(--color-brand-accent)" />
                <StatCard label="New" value={items.filter((i) => i.status === "new").length} color="#3B82F6" />
            ...
            {loading ? (
                <div className="py-24 flex items-center justify-center">
                    <FiLoader className="w-6 h-6 text-brand-accent animate-spin" />
                </div>
            ) : items.length === 0 ? (
                <div className="py-24 text-center border border-dashed border-gray-200 rounded-2xl text-gray-500">
                    No course inquiries match this filter.
                </div>
            ) : (
                                        ...
                                        <tr key={it._id} className="hover:bg-gray-50/60">
                                            ...
                                            <td className="px-4 py-3">
                                                ...
                                                <p className="text-xs text-gray-500 flex items-center gap-1">
                                                    <FiPhone size={11} /> {it.phone}
                                                </p>
                                            </td>
                                            <td className="px-4 py-3 max-w-[260px]">
                                                <p className="text-sm text-gray-800 truncate" title={ex.courseTitle}>
                                                    {ex.courseTitle || "—"}
                                                </p>
                                                <p className="text-[11px] text-gray-500 mt-0.5">
                                                    {ex.categoryName || "—"}
                                                    {ex.price ? ` · ${Number(ex.price).toLocaleString()} ${ex.currency || "BDT"}` : ""}
                                                </p>
                                            </td>
                                            <td className="px-4 py-3">
                                                <span
                                                    className="text-xs px-2 py-1 rounded-md font-semibold capitalize"
                                                    style={
                                                        ex.courseType === "offline"
                                            ...
                                            <td className="px-4 py-3">
                                                {it.source === "whatsapp" ? (
                                                    <span className="inline-flex items-center gap-1 text-xs font-semibold text-whatsapp-hover">
...
// ── Detail modal — full submitted payload ──
function DetailModal({ it, onClose }) {
    const ex = it.extra || {};
    const waHref = it.phone
        ? `https://wa.me/${it.phone.replace(/\D/g, "")}?text=${encodeURIComponent(
              `Hi ${it.name}, regarding your admission inquiry ${inqNo(it._id)} for "${ex.courseTitle || "our course"}"...`
          )}`
        : null;
            ...
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
                <div className="sticky top-0 bg-white border-b border-gray-100 p-5 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-xs text-gray-500">Course Inquiry</p>
                ...
                <div className="p-5 space-y-5">
                    <Section title="Applicant">
                        ...
                        <Row label="Phone">
                            <a href={`tel:${it.phone}`} className="text-brand-blue font-semibold">{it.phone}</a>
                        </Row>
                        {it.email && (
                            <Row label="Email">
                                <a href={`mailto:${it.email}`} className="text-brand-blue">{it.email}</a>
                            </Row>
                        )}
                    </Section>

                    <Section title="Course">
                        <Row label="Course">{ex.courseTitle}</Row>
                        <Row label="Category">{ex.categoryName}</Row>
                        <Row label="Type">
                            <span className="capitalize">{ex.courseType}</span>
                        </Row>
                        {ex.courseType === "offline" && ex.location && <Row label="Location">{ex.location}</Row>}
                        {ex.duration && <Row label="Duration">{ex.duration}</Row>}
                        {ex.startDate && <Row label="Batch starts">{ex.startDate}</Row>}
                        {ex.classSchedule && <Row label="Schedule">{ex.classSchedule}</Row>}
                        {ex.price ? (
                            <Row label="Fee">
                                <span className="font-bold text-brand-accent">
                                    {Number(ex.price).toLocaleString()} {ex.currency || "BDT"}
                                </span>
                            </Row>
                        ) : null}
                        {ex.courseSlug && (
                            <Row label="Public page">
                                <a
                                    href={`/courses/${ex.courseSlug}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-1 text-brand-blue text-xs font-semibold"
                                >
                                    /courses/{ex.courseSlug} <FiExternalLink size={11} />
                                </a>
                            </Row>
                        )}
                    </Section>
                    ...
                    {ex.note && (
                        <Section title="Applicant's note">
                            <p className="px-3 py-2 text-sm text-gray-700">{ex.note}</p>
                        </Section>
                    )}

                    {it.message && (
                        <Section title="Full submitted message">
                    ...
                    <Section title="Meta">
                        ...
                        <Row label="Via">
                            <span className="capitalize">{it.source || "website"}</span>
                        </Row>
                        <Row label="Status">
                        ...
                        {it.pageUrl && (
                            <Row label="Source page">
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-brand-blue text-xs truncate"
                                >
                                    {it.pageUrl}
                                </a>
                            </Row>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                        {waHref && (
                            <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-whatsapp hover:bg-whatsapp-hover text-white text-sm font-bold"
                            >
                        ...
                        {it.email && (
                            <a
                                href={`mailto:${it.email}?subject=Re: Course Admission Inquiry ${inqNo(it._id)}`}
                                className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-brand-dark hover:bg-brand-dark-hover text-white text-sm font-bold"
                            >
...
function Row({ label, children }) {
    return (
        <div className="flex items-start gap-3 px-3 py-2 text-sm">
            <span className="text-[11px] font-semibold text-gray-500 w-24 flex-shrink-0 pt-0.5">{label}</span>
            <span className="text-brand-dark font-medium break-words min-w-0">{children ?? "—"}</span>
        </div>
    );
}
