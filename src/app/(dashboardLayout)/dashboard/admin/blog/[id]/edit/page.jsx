...
// Shared SEO editor — same component the create page uses.
...
export default function EditBlogPage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = {
                ...
                // Keywords typed as comma-separated string; backend expects array.
    ...
    if (loading) {
        return <div className="flex justify-center items-center min-h-[60vh]"><FiLoader className="animate-spin text-brand-dark" size={32} /></div>;
    }
        ...
        <div className="p-6 lg:p-8">
            ...
            <form onSubmit={handleSubmit} className="max-w-5xl">
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-brand-dark focus:ring-1 focus:ring-brand-dark/10" required />
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <textarea value={formData.excerpt} onChange={(e) => setFormData({ ...formData, excerpt: e.target.value })} rows={3} className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm outline-none resize-none focus:border-brand-dark focus:ring-1 focus:ring-brand-dark/10" placeholder="Brief summary..." />
                            </div>
                            <div>
                    ...
                    <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
                        ...
                        {thumbnailMode === "upload" ? (
                            <label className={`flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-xl cursor-pointer transition-all ${thumbnailUploading ? "border-brand-accent bg-brand-accent/5" : "border-gray-200 hover:border-brand-dark hover:bg-gray-50"}`}>
                                {thumbnailUploading ? (
                                    <div className="flex items-center gap-2 text-brand-accent"><FiLoader className="animate-spin" size={20} /><span className="text-sm font-semibold">Uploading...</span></div>
                                ) : (
                ...
                <div className="flex gap-4 mt-6">
                    ...
                    <button type="submit" disabled={saving} className="flex-1 flex items-center justify-center gap-2 py-3 bg-brand-accent text-white rounded-lg text-sm font-semibold hover:bg-[#d67a20] disabled:bg-gray-300 transition-colors cursor-pointer">
